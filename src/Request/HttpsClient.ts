...
export default class HttpsClient {
    ...
    public async sendRequest<R extends HttpResponseConstructor = HttpResponseConstructor>(url: URL, data: object, options: RequestConfig, responseClass: R): Promise<InstanceType<R>> {
        try {
            const config = {
                url: url.toString(),
                ...this._options,
